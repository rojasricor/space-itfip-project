 ...
 * @var [type]
 * strtolower convierte a letras minuscu
 */
...
if (!$cedula
    ...
    ||
    !is_numeric($cedula)) {
    header(sprintf('Location: registrar.php?cedula_invalida&err=%s', $_POST['cedula']));
    exit;
}
...
if (!$telefono
    ...
    ||
    !is_numeric($telefono)) {
    header(sprintf('Location: registrar.php?telefono_invalido&err=%s', $_POST['telefono']));
    exit;
}
 ...
 * @var [type]
 */
$respuesta = PrestamosControlador::verificarSiEstaDisponible($fecha_entrega_bien, $fecha_fin_bien);
/**
 * [$esta_disponible description]
 * @var [type]
 */
$esta_disponible = $respuesta['disponibilidad'];
